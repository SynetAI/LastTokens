// apim-1 / pkg-13 Host half（v7.3）
// 用法：将本文件内容作为 cordis_define 的 code.host（plain JS function body）。
// v7.3：按日/按小时聚合账本（全量历史，独立于 200 条记录截断），记录携带所用单价 prices。
return {
  inject: ['timer'],
  apply(ctx) {
    // ── 账本状态（进程内） ────────────────────────────────
    ...
    // ── 按日 / 按小时聚合（key：'YYYY-MM-DD' / 'YYYY-MM-DD HH'，本地时区） ──
    const daily = {}
    const hourly = {}
    const pad2 = (n) => String(n).padStart(2, '0')
    function addToAgg(map, key, rec) {
      const e = map[key] || (map[key] = { calls: 0, cost: 0, inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0 })
      e.calls += 1
      e.cost += rec.cost
      e.inputTokens += rec.inputTokens
      e.outputTokens += rec.outputTokens
      e.cacheReadTokens += rec.cacheReadTokens
      e.cacheWriteTokens += rec.cacheWriteTokens
    }
    function aggList(map, limit) {
      return Object.keys(map).sort().slice(-limit).map((k) => Object.assign({ key: k }, map[k]))
    }
    ...
    function recordCall(options, usage, startedAt, status, errMsg) {
      const u = usage || {}
      const p = priceFor(options.model)
      const rec = {
        id: nextId++,
        ts: startedAt,
        ...
        prices: { input: p.input, hit: p.hit, output: p.output },
        status,
        error: errMsg,
      }
      records.push(rec)
      ...
      const hourKey = dayKey + ' ' + pad2(d.getHours())
      addToAgg(daily, dayKey, rec)
      addToAgg(hourly, hourKey, rec)
      const dKeys = Object.keys(daily)
      if (dKeys.length > 35) dKeys.slice(0, dKeys.length - 35).forEach((k) => { delete daily[k] })
      const hKeys = Object.keys(hourly)
      if (hKeys.length > 60) hKeys.slice(0, hKeys.length - 60).forEach((k) => { delete hourly[k] })
    }
    ...
    harness.handle('snapshot', () => ({
      ...
      daily: aggList(daily, 14),
      hourly: aggList(hourly, 24),
    }))
